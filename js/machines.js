...
const MACHINES = {
  eva: {
    ...
    // 「電チュー・割合」：ST中（右打ち中）の当り内訳（合計1.0）。
    // 8R×4回・約4,800個がレアで0.5%、8R×2回・約2,400個が99.5%。
    // 時短からのST引き戻し当り（時短中＝左打りでのヒット）はこの分岐の対象外で従来通り2,400個固定。
    st: {
      bigRate: 0.005, // ST中の当りのうち8R×4回・約4,800個になる割合
    },
    ...
    payout: {
      ...
      stWin: 2400, // ST中の大当り（8R×2回。時短経由でST突入した際の当りも含む）
      stWinBig: 4800, // ST中の大当り（レア、8R×4回）
    },
  },
  ...
  sao: {
    ...
    // 「次回まで」22.5%は10R・約1,500個かつ回転数無制限（無限ST、次に当たるまで終わらない）。
    // 「ST115回転」47.5%は同じく10R・約1,500個だが119回転のカウントダウン。
    // 残り30%は2R・約300個・119回転のカウントダウン。
    lightning: {
      infiniteRate: 0.225, // 10R・約1,500個・無限ST（「次回まで」）
      highRate: 0.475, // 10R・約1,500個・119回転カウントダウン（「ST115回転」）
      lowRate: 0.3, // 2R・約300個・119回転カウントダウン
    },
